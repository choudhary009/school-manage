...
const { createInitialPasskeysForAdmin } = require('./license-controller.js');
...
const adminRegister = async (req, res) => {
    try {
        ...
        if (existingAdminByEmail) {
            ...
            let result = await admin.save();
            // auto-create initial online + offline passkeys for this admin
            try {
                await createInitialPasskeysForAdmin(result._id);
            } catch (e) {
                console.error("Failed to create initial passkeys:", e.message);
            }
            result.password = undefined;
            res.send(result);
        }
