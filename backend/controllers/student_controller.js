...
const studentRegister = async (req, res) => {
  try {
    ...
    const optionalFields = ['email', 'mobilePhone', 'whatsappNumber', 'studentCNIC', 'fatherOccupation', 
                           'fatherContactNumber', 'fatherCNIC', 'motherContactNumber', 'motherCNIC',
                           'height', 'weight', 'street', 'houseNumber', 'city', 'studentImage', 'registerNumber'];
    ...
    if (!cleanedBody.sclassName) {
      return res.status(400).json({ message: "Class is required" });
    }
    if (cleanedBody.rollNum === undefined || cleanedBody.rollNum === null || cleanedBody.rollNum === '') {
      return res.status(400).json({ message: "Roll Number is required" });
    }
    
    // Convert rollNum to number
    const rollNum = typeof cleanedBody.rollNum === 'string' ? parseInt(cleanedBody.rollNum, 10) : cleanedBody.rollNum;
    if (isNaN(rollNum)) {
