...
async function markAllStudentAttendance(req, res) {
  ...
  try {
    for (const item of attendanceArray) {
      try {
        const updatedItem = await Student.findByIdAndUpdate(
          item.id,
          {
            $push: {
              attendance: {
        ...
        console.log(`Attendance updated for student with ID ${item._id}`);
      } catch (error) {
        console.error(
          `Error updating attendance for student with ID ${item._id}:`,
          error
        );
      }
    }
