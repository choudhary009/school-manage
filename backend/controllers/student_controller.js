...
const studentRegister = async (req, res) => {
  try {
    ...
    // Sync discount and discountFee for consistency
    if (studentData.discount !== undefined && (studentData.discountFee === undefined || studentData.discountFee === 0)) {
      studentData.discountFee = studentData.discount;
    } else if (studentData.discountFee !== undefined && (studentData.discount === undefined || studentData.discount === 0)) {
      studentData.discount = studentData.discountFee;
    }
...
const getStudents = async (req, res) => {
  try {
    ...
    let students = await Student.find(query)
      ...
      .select("+mobilePhone +whatsappNumber +email");
...
const getStudentDetail = async (req, res) => {
  try {
    let student = await Student.findById(req.params.id)
      ...
      .select("+email +mobilePhone +whatsappNumber"); // Explicitly include email, mobilePhone and whatsappNumber fields
    if (student) {
      student.password = undefined;
...
const updateStudent = async (req, res) => {
  try {
    ...
    const fieldsToUpdate = [
      ...
      'siblingId', 'totalFee', 'discountFee', 'studentFee', 'status'
    ];
    ...
    // Sync discount and discountFee for consistency
    if (updateData.discount !== undefined && updateData.discountFee === undefined) {
      updateData.discountFee = updateData.discount;
    } else if (updateData.discountFee !== undefined && updateData.discount === undefined) {
      updateData.discount = updateData.discountFee;
    }
...
const getSiblingIds = async (req, res) => {
  try {
    const { schoolId } = req.params;
    const siblingIds = await Student.distinct("siblingId", {
      school: schoolId,
      siblingId: { $ne: null, $ne: "" }
    });
    res.status(200).json({ ok: true, data: siblingIds });
  } catch (error) {
    res.status(500).json({ ok: false, message: error.message });
  }
};

const getStudentsBySiblingId = async (req, res) => {
  try {
    const { schoolId, siblingId } = req.params;
    const students = await Student.find({ school: schoolId, siblingId: siblingId })
      .populate("sclassName", "sclassName sclassSection")
      .select("-password");
