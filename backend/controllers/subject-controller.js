...
const classSubjects = async (req, res) => {
  try {
    let subjects = await Subject.find({ sclassName: req.params.id }).populate("teacher", "name teacherImage");
    if (subjects.length > 0) {
      res.send(subjects);
    } else {
      res.send({ message: "No subjects found" });
...
async function getClassesSubjects(req, res) {
  ...
  try {
    ...
    res.status(200).json({ data: allSubjects, ok: true, msg: "Data fetched" });
  } catch (error) {
