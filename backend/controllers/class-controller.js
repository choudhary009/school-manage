  ...
  try {
    ...
    if (isValidObjectId) {
      students = await Student.find({ sclassName: classId })
        .select("name rollNum gender sclassName examResult fatherName discount discountFee totalFee studentFee siblingId status mobilePhone whatsappNumber")
...
async function getClassesStudents(req, res) {
  ...
  try {
    ...
    res.status(200).json({ data: allStudents, ok: true, message: "Data fetched" });
  } catch (error) {
