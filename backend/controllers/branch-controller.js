...
const getBranchStudents = async (req, res) => {
  try {
    ...
    const students = await Student.find({ school: branchId })
...
const getBranchTeachers = async (req, res) => {
  try {
    ...
    const teachers = await Teacher.find({ school: branchId })
      .select('-password -teacherImage -cvImage')
...
// Get branch subjects
const getBranchSubjects = async (req, res) => {
  try {
    const { branchId } = req.params;
    ...
    const subjects = await Subject.find({ school: branchId })
      .populate('sclassName', 'sclassName sclassSection')
