...
const teacherRegister = async (req, res) => {
  ...
  if (!name || !email) {
    return res.status(400).json({ message: "Name and Email are required", ok: false });
  }

  try {
    // Check for existing email first (before hashing password)
    const existingTeacherByEmail = await Teacher.findOne({ email });
    if (existingTeacherByEmail) {
      return res.status(400).json({ message: "Email already exists" });
    }

    const rawPassword = req.body.password || '12345678';
...
async function filterTeacherClassSubjects(req, res) {
  ...
  try {
    const data = await Subject.find({ sclassName }).populate("teacher", "name");
    res.status(200).json({
      data: data,
      ok: true,
