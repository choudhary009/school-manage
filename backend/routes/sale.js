...
// Company: Get all sales (with filtering, pagination)
  ...
  try {
    ...
    if (search) {
      query.$or = [
        ...
        { sellerNameUrdu: { $regex: search, $options: 'i' } },
        { supplierName: { $regex: search, $options: 'i' } },
        { supplierNameUrdu: { $regex: search, $options: 'i' } },
...
router.get('/company/:saleId', verifyCompany, async (req, res) => {
  try {
    ...
    // Validate ObjectId to avoid CastError when bad ids like "[object Object]" are passed
    if (!mongoose.Types.ObjectId.isValid(saleId)) {
      return res.status(400).json({ message: 'Invalid sale id', saleId });
    }
...
router.put('/company/:saleId', verifyCompany, async (req, res) => {
  try {
    ...
    if (billId !== undefined) sale.billId = billId || null;
