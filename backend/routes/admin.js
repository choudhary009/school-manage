...
router.post('/companies', verifyAdmin, async (req, res) => {
  try {
    ...
    const company = new Company({
      ...
      onlinePassKey: onlinePassKey || '',
      offlinePassKeyExpiresAt: offlinePassKeyExpiresAt
        ? new Date(offlinePassKeyExpiresAt)
        : null,
      onlinePassKeyExpiresAt: onlinePassKeyExpiresAt
        ? new Date(onlinePassKeyExpiresAt)
        : null
    });
...
router.put('/companies/:id', verifyAdmin, async (req, res) => {
  try {
    ...
    if (billHeaderImage !== undefined) company.billHeaderImage = billHeaderImage;
    if (dashboardType) company.dashboardType = dashboardType;
    if (offlinePassKey !== undefined) company.offlinePassKey = offlinePassKey;
    if (onlinePassKey !== undefined) company.onlinePassKey = onlinePassKey;
    if (offlinePassKeyExpiresAt !== undefined) {
      company.offlinePassKeyExpiresAt = offlinePassKeyExpiresAt
        ? new Date(offlinePassKeyExpiresAt)
        : null;
    }
    if (onlinePassKeyExpiresAt !== undefined) {
      company.onlinePassKeyExpiresAt = onlinePassKeyExpiresAt
        ? new Date(onlinePassKeyExpiresAt)
        : null;
    }
...
// Update company bill header image
router.put('/companies/:id/bill-header-image', verifyAdmin, async (req, res) => {
  try {
    const { billHeaderImage } = req.body;
    ...
    if (billHeaderImage !== undefined) {
      company.billHeaderImage = billHeaderImage;
    }

    await company.save();

    res.json({
      message: 'Bill header image updated successfully',
      company: {
        id: company._id,
        billHeaderImage: company.billHeaderImage
      }
    ...
    console.error('Update bill header image error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});
...
// Get company page permissions
router.get('/companies/:id/page-permissions', verifyAdmin, async (req, res) => {
  try {
    const company = await Company.findById(req.params.id).select('pagePermissions');
    ...
    const permissions = {};
    if (company.pagePermissions) {
      company.pagePermissions.forEach((value, key) => {
...
// Update company page permissions
router.put('/companies/:id/page-permissions', verifyAdmin, async (req, res) => {
  try {
    ...
    if (pagePermissions && typeof pagePermissions === 'object') {
      if (!company.pagePermissions) {
        company.pagePermissions = new Map();
      }
      
      Object.keys(pagePermissions).forEach(key => {
        company.pagePermissions.set(key, pagePermissions[key] === true);
      });
    }
    ...
    console.error('Update page permissions error:', error);
    res.status(500).json({ message: 'Server error', error: error.message });
  }
});

// Get recent device sessions for a company
router.get('/companies/:id/devices', verifyAdmin, async (req, res) => {
  try {
    const sessions = await CompanyDeviceSession.find({
    ...
    console.error('Get company device sessions error:', error);
    res
      .status(500)
