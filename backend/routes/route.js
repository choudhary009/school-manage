...
router.get("/", (req, res) => {
  return res.status(200).json({
    ...
    auth: { unifiedLogin: "POST /UnifiedLogin (same as POST /unifiedlogin)" },
  });
});
...
const { unifiedLogIn } = require("../controllers/unifiedAuthController.js");
...
router.post("/UnifiedLogin", unifiedLogIn);
router.post("/unifiedlogin", unifiedLogIn);
...
// Admin license / passkeys
router.get("/AdminPasskeys/:adminId", getAdminPasskeys);
router.put("/Passkey/:id", updatePasskey);
router.post("/Passkey/validate", validatePasskey);
...
router.get("/getSiblingIds/:schoolId", getSiblingIds);
router.get("/getStudentsBySiblingId/:schoolId/:siblingId", getStudentsBySiblingId);
...
// Inventory Record (admin-scoped tabs + rows)
const {
  getInventoryRecord,
  upsertInventoryRecord,
} = require("../controllers/inventoryRecordController.js");

router.get("/InventoryRecord/:adminId", getInventoryRecord);
router.put("/InventoryRecord/:adminId", upsertInventoryRecord);

// Expense Record (separate data from Inventory Record)
const {
  getExpenseRecord,
  upsertExpenseRecord,
} = require("../controllers/expenseRecordController.js");

router.get("/ExpenseRecord/:adminId", getExpenseRecord);
router.put("/ExpenseRecord/:adminId", upsertExpenseRecord);
