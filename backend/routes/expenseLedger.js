...
// Company: Get all expense ledger entries (with pagination)
router.get('/company/all', verifyCompany, async (req, res) => {
  try {
    const companyId = req.user.id;
    const { dateFrom, dateTo, page, limit = 50 } = req.query;
    ...
    const total = await ExpenseLedger.countDocuments(query);
    const limitNum = Math.min(parseInt(limit, 10) || 50, 1000);
    const pageNum = parseInt(page, 10) || 1;
    const skip = (pageNum - 1) * limitNum;

    const expenses = await ExpenseLedger.find(query)
